...
import { ApiService } from 'src/app/services/api/api.service';import { Peenginer } from 'src/app/models/pengineer.model';
import { StorageService } from 'src/app/services/storage/storage.service';

@Component({
  selector: 'app-peengineerfilterpage',
  templateUrl: './peengineerfilterpage.page.html',
  styleUrls: ['./peengineerfilterpage.page.scss'],
})
export class peengineerFilterpagePage implements OnInit {

  filterform: FormGroup;
  clientList: Peenginer[];
  requestType: string;
  ...
  segmentValue: any;
  designerList: Peenginer[];
  analystsList: Peenginer[];
  assignedId: any;
  ...
  getCompanies() {
    ...
    this.apiservice.getEngineer(userId).subscribe((res: any) => {
  ...
  clientSelected(event) {
    console.log('company.value', event.detail.value)
    this.clientId = event.detail.value.id;
    this.value = event.detail.value.firstname + event.detail.value.lastname;
  ...
  filterCompany(event) {
    ...
    if (searchValue.length > 0) {
      this.clientList = this.companyList.filter((client: any) => {
        return client.firstname.toLowerCase().indexOf(searchValue.toLowerCase()) > -1;
      });
