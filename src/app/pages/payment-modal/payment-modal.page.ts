...
export class PaymentModalPage implements OnInit {
  ...
  netPay:any
    ...
    newprelimscount = 0;
    designData:any;
    fulldesigndata: DesginDataModel;
    delivertime:String;
    designType:any;
    wattmonkadmins: any;
    clientadmins: any;
  ...
  getWattmonkAdmins(){
    this.apiService.getadmins().subscribe(res=>{
      ...
      this.apiService.getGroupTeamHead(this.fulldesigndata.creatorparentid).subscribe((response)=>{
        if(response){
  ...
  getClientAdmins(design){
    this.apiService.getclientadmins(design.creatorparentid).subscribe(res=>{
      // getGroupTeamHead(res.id);
      this.createChatGroup(design);
  ...
  confirm(){
    ...
    postData = {
      ...
      {
          this.apiService.updateDesignForm(postData,this.id).subscribe((value:DesginDataModel)=>{
            if(this.design=='prelim')
        {
          // this.createChatGroup(value);
          this.getClientAdmins(value);
    ...
    this.clientadmins.forEach(element => {
      clientadminsid.push(element)
    });
  
      CometChat.createGroup(group).then(group=>{
        let membersList = [];
        if(this.userData.role.type!='clientadmin'||this.userData.role.type!='clientsuperadmin'){
          membersList=[new CometChat.GroupMember("" + this.fulldesigndata.createdby.cometchatuid, CometChat.GROUP_MEMBER_SCOPE.ADMIN)]
        }
